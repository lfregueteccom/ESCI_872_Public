...
def absorption( t=None,d=None,s=None,pH=None,f=None):
    ...
    if not -7.7 < pH < 8.3:
        raise RuntimeError('absorption.absorption: Acidity out of range')
    if not 5 < s < 50:
        raise RuntimeError('absorption.absorption: Salinity out of range!')
     
    try:
        if not 0 < d.all() < 7:
            raise RuntimeError('absorption.absorption: Depth out of range!')
    except:
        if not 0 < d < 7:
            raise RuntimeError('absorption.absorption: Depth out of range!')
    ...
    a += 0.106 * f1 * f ** 2 / (f1 ** 2 + f ** 2) * exp((pH - 8) / 0.56)
    
    # C2.1.7 Ainslie and McColm Simplified Formula: Magnesium Sulphate
    a += .52 * (1 + t/43) * s/35 * (f2 * f ** 2) / (f2 ** 2 + f ** 2) * exp(-1 * d / 6)
...
def absorption_draw(verbose = True):
    ...
    # # Determine the Absorption Generated by Magnesium Sulfate
    a_MgSO_4 = 0.52*(1+t/43)*s/35*f2*f**2/(f2**2+f**2)*exp(-d/6)
